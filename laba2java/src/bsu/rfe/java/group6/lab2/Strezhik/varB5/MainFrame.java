package bsu.rfe.java.group6.lab2.Strezhik.varB5;
...
public class MainFrame extends JFrame {
    ...
    public Double calculate1(Double x, Double y, Double z) {
        return Math.pow((Math.cos(Math.exp(x))+Math.pow(Math.log(1+y), 2)+
                Math.sqrt(Math.exp(Math.cos(x))+Math.pow(Math.sin(Math.PI*z),2))+
   Ф             Math.sqrt(1/x))+Math.cos(y)*(Math.cos((y))),Math.sin(z));
    }
